...
echo "🔄 Starting materialized view migration..."

# Drop existing materialized views
echo "1️⃣ Dropping block_proposal_metrics_hourly_mv..."
docker exec -it monad-clickhouse clickhouse-client -d monad_analytics --query "DROP VIEW IF EXISTS block_proposal_metrics_hourly_mv"

echo "2️⃣ Dropping qc_participation_metrics_hourly_mv..."
docker exec -it monad-clickhouse clickhouse-client -d monad_analytics --query "DROP VIEW IF EXISTS qc_participation_metrics_hourly_mv"

# Recreate block_proposal_metrics_hourly_mv with COALESCE for NULL handling
echo "3️⃣ Recreating block_proposal_metrics_hourly_mv..."
docker exec -it monad-clickhouse clickhouse-client -d monad_analytics --query "
...
echo "4️⃣ Recreating qc_participation_metrics_hourly_mv..."
docker exec -it monad-clickhouse clickhouse-client -d monad_analytics --query "
...
docker exec -it monad-clickhouse clickhouse-client -d monad_analytics --query "SHOW TABLES LIKE '%_mv'"
